...
func NewRemoteDB(serverAddr string, serverKey string) (*RemoteDB, error) {
	...
	return newRemoteDB(fmt.Sprintf("%s://%s", serverAddr, serverKey), gdc, err)
}

func newRemoteDB(name string, gdc protodb.DBClient, err error) (*RemoteDB, error) {
	if err != nil {
		return nil, err
	}
...
func (rd *RemoteDB) PrefixIterator(prefix []byte) (db.Iterator, error) {
	start, end, err := db.PrefixToRange(prefix)
	if err != nil {
		return nil, err
	}
...
func (rd *RemoteDB) ReversePrefixIterator(prefix []byte) (db.Iterator, error) {
	start, end, err := db.PrefixToRange(prefix)
	if err != nil {
		return nil, err
	}
